<?php
/**
 * This file contains the functions needed to properly install the Echo Band Theme Bundle.
 *
...
function mp_theme_bundle_activation_set_up_plugin(){
	...
	deactivate_plugins( $plugins_dir . '/echo-band-theme-bundle/echo-band-theme-bundle.php' );

	//Create a zip containing this "theme" in the plugins directory
	mp_core_zip_directory( realpath( get_template_directory() ), $plugins_dir . '/echo-band-theme-bundle.zip' );

	//Unzip it
	$unzip_result = unzip_file( $plugins_dir . '/echo-band-theme-bundle.zip', $plugins_dir . '/' );

	//Delete the temp zipped file
	$wp_filesystem->rmdir( $plugins_dir . '/echo-band-theme-bundle.zip' );

	//Now that the plugin is where it needs to be, activate it.
	activate_plugin( $plugins_dir . '/echo-band-theme-bundle/echo-band-theme-bundle.php' );
