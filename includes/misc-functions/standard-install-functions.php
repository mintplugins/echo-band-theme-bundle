<?php
/**
 * Installaion Functions for the Echo Band Theme Bundle. This file is standard for any Theme Bundle and should be left alone other than find and replacing.
 * When building new Theme Bundles, simple do a find and replace for each of the following and save:
 *
 * Find and replace: ECHO
 ...
 */
require( ECHO_BAND_THEME_BUNDLE_PLUGIN_DIR . 'includes/misc-functions/class-parent-plugin-installation.php' );
require( ECHO_BAND_THEME_BUNDLE_PLUGIN_DIR . 'includes/misc-functions/custom-install-functions.php' );
 ...
 * @return void
 */
function echo_band_theme_bundle_install() {
	...
	$mp_core_options['mp_stacks_theme_bundle_being_installed'] = array( 
		'theme_bundle_slug' => 'echo_band_theme_bundle',
		'fancy_title' => 'Echo Band Theme Bundle',
		'support_url' => 'https://mintplugins.com/support/echo-band-theme-bundle-support/',
		'support_email' => 'support@mintplugins.com',
		'required_theme_dirname' => apply_filters( 'echo_band_theme_bundle_required_theme_dirname', 'knapstack' ),
...
register_activation_hook( ECHO_BAND_THEME_BUNDLE_PLUGIN_FILE, 'echo_band_theme_bundle_install' );
 ...
 * @return void
 */
function echo_band_theme_bundle_admin_init(){
		
	//Set up the auto redirects to install dependant plugins
	global $mp_core_options, $echo_band_theme_bundle_installation_routine;
...
add_action( 'init', 'echo_band_theme_bundle_admin_init' );
 ...
 */
function echo_band_theme_bundle_initialize_default_items_setup( $mp_core_options, $theme_bundle_slug ){
	
	if ( $theme_bundle_slug == 'echo_band_theme_bundle' ){
...
add_filter( 'mp_core_parent_plugin_installation_complete_filter', 'echo_band_theme_bundle_initialize_default_items_setup', 10, 2 );
 ...
 * @return   void
 */
function echo_band_theme_bundle_switch_theme_workaround(){		
	...
	$fancy_theme_name = apply_filters( 'echo_band_theme_bundle_fancy_theme_name', NULL );
...
add_action( 'wp_die_handler', 'echo_band_theme_bundle_switch_theme_workaround' );
 ...
 * @return void
 */
function echo_band_theme_bundle_kill_edd_welcome(){
...
add_action( 'plugins_loaded', 'echo_band_theme_bundle_kill_edd_welcome' );